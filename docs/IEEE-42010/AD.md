...
|----|-------------|--------|--------|------------|
| GAP-01 | Elicitation patterns documented with FastMCP v3 typed API | **Closed** | docs/03f-elicitation-patterns.md | v3.0.0: FastMCP v3 `ctx.elicit(message, response_type=Type)` with `Context` injection; typed result pattern matching (`AcceptedElicitation`/`DeclinedElicitation`/`CancelledElicitation`); response types: `str`, `int`, `bool`, `Literal`, `list` (multi-select), `dict` (titled options), `dataclass`, `BaseModel`; default values via `Field(default=...)`; multi-turn elicitation; confirmation (`response_type=None`); FR-ELIC-001–005 traced |
...
| GAP-03 | Multi-server orchestration with FastMCP v3 composition | **Closed** | docs/03h-multi-server-orchestration.md | v2.0.0: FastMCP v3 Server Composition section with `mount()` (live link), `import_server()` (static copy), `create_proxy()` (remote HTTP, subprocess, npm/uvx); namespacing transforms (tools, resources, prompts); tag-based capability filtering (`include_tags`/`exclude_tags`); direct vs proxy mounting trade-offs; composition architecture diagram; FR-ORCH-001–005 traced |
